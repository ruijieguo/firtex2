...
static int r_main_suffix(struct SN_env * z) {
    ...
    {   int m3; /* setlimit, line 37 */
        ...
        z->c = z->l - m;
        z->ket = z->c; /* [, line 37 */
        if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((1851442 >> (z->p[z->c - 1] & 0x1f)) & 1)) { z->lb = m3; return 0; }
...
static int r_consonant_pair(struct SN_env * z) {
    {   int m3; /* setlimit, line 50 */
        ...
        {   int m = z->l - z->c; (void) m; /* and, line 52 */
            if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((1064976 >> (z->p[z->c - 1] & 0x1f)) & 1)) { z->lb = m3; return 0; }
...
static int r_other_suffix(struct SN_env * z) {
    ...
    {   int m3; /* setlimit, line 55 */
        ...
        z->c = z->l - m;
        z->ket = z->c; /* [, line 56 */
        if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((1572992 >> (z->p[z->c - 1] & 0x1f)) & 1)) { z->lb = m3; return 0; }
