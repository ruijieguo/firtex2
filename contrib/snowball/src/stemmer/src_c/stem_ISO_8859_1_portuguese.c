...
static int r_prelude(struct SN_env * z) {
    ...
    while(1) { /* repeat, line 36 */
        int c = z->c;
        z->bra = z->c; /* [, line 37 */
        if (z->c + 0 >= z->l || (z->p[z->c + 0] != 227 && z->p[z->c + 0] != 245)) among_var = 3; else
...
static int r_standard_suffix(struct SN_env * z) {
    int among_var;
    z->ket = z->c; /* [, line 77 */
    if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((839714 >> (z->p[z->c - 1] & 0x1f)) & 1)) return 0;
    ...
    switch(among_var) {
        ...
        case 5:
            ...
            {   int m = z->l - z->c; (void) m; /* try, line 111 */
                z->ket = z->c; /* [, line 112 */
                if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((4718616 >> (z->p[z->c - 1] & 0x1f)) & 1)) { z->c = z->l - m; goto lab0; }
        ...
        case 6:
            ...
            {   int m = z->l - z->c; (void) m; /* try, line 123 */
                z->ket = z->c; /* [, line 124 */
                if (z->c <= z->lb || (z->p[z->c - 1] != 101 && z->p[z->c - 1] != 108)) { z->c = z->l - m; goto lab1; }
        ...
        case 7:
            ...
            {   int m = z->l - z->c; (void) m; /* try, line 135 */
                z->ket = z->c; /* [, line 136 */
                if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((4198408 >> (z->p[z->c - 1] & 0x1f)) & 1)) { z->c = z->l - m; goto lab2; }
