...
static int r_prelude(struct SN_env * z) {
    ...
    {   int c_test = z->c; /* test, line 42 */
        while(1) { /* repeat, line 42 */
            int c = z->c;
            z->bra = z->c; /* [, line 43 */
            if (z->c + 0 >= z->l || z->p[z->c + 0] >> 5 != 7 || !((340306450 >> (z->p[z->c + 0] & 0x1f)) & 1)) among_var = 6; else
...
static int r_postlude(struct SN_env * z) {
    ...
    while(1) { /* repeat, line 75 */
        int c = z->c;
        z->bra = z->c; /* [, line 77 */
        if (z->c + 0 >= z->l || (z->p[z->c + 0] != 73 && z->p[z->c + 0] != 89)) among_var = 3; else
...
static int r_undouble(struct SN_env * z) {
    {   int m_test = z->l - z->c; /* test, line 91 */
        if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((1050640 >> (z->p[z->c - 1] & 0x1f)) & 1)) return 0;
...
static int r_standard_suffix(struct SN_env * z) {
    ...
    {   int m = z->l - z->c; (void) m; /* do, line 126 */
        z->ket = z->c; /* [, line 127 */
        if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((264336 >> (z->p[z->c - 1] & 0x1f)) & 1)) goto lab4;
    ...
    {   int m = z->l - z->c; (void) m; /* do, line 146 */
        ...
        {   int m_test = z->l - z->c; /* test, line 148 */
            if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((2129954 >> (z->p[z->c - 1] & 0x1f)) & 1)) goto lab9;
