...
static int r_standard_suffix(struct SN_env * z) {
    ...
    {   int m = z->l - z->c; (void) m; /* do, line 84 */
        z->ket = z->c; /* [, line 85 */
        if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((1327104 >> (z->p[z->c - 1] & 0x1f)) & 1)) goto lab1;
    ...
    {   int m = z->l - z->c; (void) m; /* do, line 94 */
        z->ket = z->c; /* [, line 95 */
        if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((1051024 >> (z->p[z->c - 1] & 0x1f)) & 1)) goto lab2;
        ...
        switch(among_var) {
            ...
            case 4:
                ...
                {   int m = z->l - z->c; (void) m; /* try, line 111 */
                    z->ket = z->c; /* [, line 112 */
                    if (z->c <= z->lb || (z->p[z->c - 1] != 103 && z->p[z->c - 1] != 104)) { z->c = z->l - m; goto lab9; }
