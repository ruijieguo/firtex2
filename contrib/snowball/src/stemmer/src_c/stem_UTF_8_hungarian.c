...
static int r_mark_regions(struct SN_env * z) {
    ...
    {   int c = z->c; /* or, line 51 */
        ...
        {   int c = z->c; /* or, line 49 */
            if (z->c + 1 >= z->l || z->p[z->c + 1] >> 5 != 3 || !((101187584 >> (z->p[z->c + 1] & 0x1f)) & 1)) goto lab4;
...
static int r_v_ending(struct SN_env * z) {
    int among_var;
    z->ket = z->c; /* [, line 61 */
    if (z->c <= z->lb || (z->p[z->c - 1] != 161 && z->p[z->c - 1] != 169)) return 0;
...
static int r_double(struct SN_env * z) {
    {   int m_test = z->l - z->c; /* test, line 68 */
        if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((106790108 >> (z->p[z->c - 1] & 0x1f)) & 1)) return 0;
...
static int r_instrum(struct SN_env * z) {
    int among_var;
    z->ket = z->c; /* [, line 77 */
    if (z->c <= z->lb || z->p[z->c - 1] != 108) return 0;
...
static int r_case_special(struct SN_env * z) {
    int among_var;
    z->ket = z->c; /* [, line 116 */
    if (z->c <= z->lb || (z->p[z->c - 1] != 110 && z->p[z->c - 1] != 116)) return 0;
...
static int r_owned(struct SN_env * z) {
    int among_var;
    z->ket = z->c; /* [, line 154 */
    if (z->c <= z->lb || (z->p[z->c - 1] != 105 && z->p[z->c - 1] != 169)) return 0;
