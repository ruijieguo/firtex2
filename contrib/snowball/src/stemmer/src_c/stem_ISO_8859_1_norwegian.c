...
static int r_main_suffix(struct SN_env * z) {
    ...
    {   int m3; /* setlimit, line 38 */
        ...
        z->c = z->l - m;
        z->ket = z->c; /* [, line 38 */
        if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((1851426 >> (z->p[z->c - 1] & 0x1f)) & 1)) { z->lb = m3; return 0; }
...
static int r_consonant_pair(struct SN_env * z) {
    {   int m_test = z->l - z->c; /* test, line 53 */
        {   int m3; /* setlimit, line 54 */
            ...
            z->c = z->l - m;
            z->ket = z->c; /* [, line 54 */
            if (z->c <= z->lb || z->p[z->c - 1] != 116) { z->lb = m3; return 0; }
...
static int r_other_suffix(struct SN_env * z) {
    ...
    {   int m3; /* setlimit, line 63 */
        ...
        z->c = z->l - m;
        z->ket = z->c; /* [, line 63 */
        if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((4718720 >> (z->p[z->c - 1] & 0x1f)) & 1)) { z->lb = m3; return 0; }
