...
static int r_adjective(struct SN_env * z) {
    int among_var;
    z->ket = z->c; /* [, line 90 */
    if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 6 || !((2271009 >> (z->p[z->c - 1] & 0x1f)) & 1)) return 0;
...
static int r_adjectival(struct SN_env * z) {
    ...
    {   int m = z->l - z->c; (void) m; /* try, line 111 */
        z->ket = z->c; /* [, line 112 */
        if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 6 || !((671113216 >> (z->p[z->c - 1] & 0x1f)) & 1)) { z->c = z->l - m; goto lab0; }
...
static int r_reflexive(struct SN_env * z) {
    int among_var;
    z->ket = z->c; /* [, line 131 */
    if (z->c <= z->lb || (z->p[z->c - 1] != 209 && z->p[z->c - 1] != 216)) return 0;
...
static int r_derivational(struct SN_env * z) {
    int among_var;
    z->ket = z->c; /* [, line 178 */
    if (z->c <= z->lb || (z->p[z->c - 1] != 212 && z->p[z->c - 1] != 216)) return 0;
