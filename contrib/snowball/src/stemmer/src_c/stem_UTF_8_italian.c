...
static int r_postlude(struct SN_env * z) {
    ...
    while(1) { /* repeat, line 70 */
        int c = z->c;
        z->bra = z->c; /* [, line 72 */
        if (z->c + 0 >= z->l || (z->p[z->c + 0] != 73 && z->p[z->c + 0] != 85)) among_var = 3; else
...
static int r_attached_pronoun(struct SN_env * z) {
    int among_var;
    z->ket = z->c; /* [, line 87 */
    if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((33314 >> (z->p[z->c - 1] & 0x1f)) & 1)) return 0;
    ...
    switch(among_var) {
        ...
        case 7:
            ...
            {   int m = z->l - z->c; (void) m; /* try, line 126 */
                z->ket = z->c; /* [, line 127 */
                if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((4722696 >> (z->p[z->c - 1] & 0x1f)) & 1)) { z->c = z->l - m; goto lab1; }
