...
void GlobalParams::parseCIDToUnicode(GList *tokens, GString *fileName,
  ...
  GString* fullPath = getFullPath(fileName,name);
  cidToUnicodes->add(collection->copy(), fullPath);
...
void GlobalParams::parseUnicodeMap(GList *tokens, GString *fileName,
  ...
  GString* fullPath = getFullPath(fileName,name);
  unicodeMaps->add(encodingName->copy(), fullPath);
...
void GlobalParams::parseCMapDir(GList *tokens, GString *fileName, int line) {
  ...
  GString* fullPath = getFullPath(fileName,dir);  
...
void GlobalParams::parseToUnicodeDir(GList *tokens, GString *fileName,
  ...
  GString* dir = (GString *)tokens->get(1);
  GString* fullPath = getFullPath(fileName,dir);  
  toUnicodeDirs->append(fullPath);
...
GString* GlobalParams::getFullPath(GString* cfgFile,GString* rPath)	
{
	int nPos = cfgFile->getLength();
	char* cf = cfgFile->getCString() + nPos - 1;	
	while ((nPos > 0) &&  (*cf != '/') && (*cf != '\\'))
	{
		cf--;
		nPos--;
	}	
	GString* fullPath = new GString();
	fullPath->append(cfgFile->getCString(),nPos);
