...
* @return 
*/
FILE* Ole::ole_init(FILE *f, void *buffer, size_t bufSize)  {
	...
	sectorSize = 1<<NumUtils::getshort(oleBuf,0x1e);
	shortSectorSize=1<<NumUtils::getshort(oleBuf,0x20);

	/* Read BBD into memory */
	bbdNumBlocks = NumUtils::getulong(oleBuf,0x2c);
	...
	mblock=NumUtils::getlong(oleBuf,0x44);
	msat_size=NumUtils::getlong(oleBuf,0x48);
	...
	while((mblock >= 0) && (i < msat_size)) {
		...
		if(fread(tmpBuf+MSAT_ORIG_SIZE+(sectorSize-4)*i,
			...
			i++;
			mblock=NumUtils::getlong(tmpBuf, MSAT_ORIG_SIZE+(sectorSize-4)*i);
	}

	/* 	fprintf(stderr, "bbdNumBlocks=%ld\n", bbdNumBlocks); */
	for(i=0; i< bbdNumBlocks; i++) {
		long int bbdSector=NumUtils::getlong(tmpBuf,4*i);
	...
	sbdLen=0;
	sbdMaxLen=10;
	sbdCurrent = sbdStart = NumUtils::getlong(oleBuf,0x3c);
	if (sbdStart > 0) {
		...
		while(1) {
			...
			size_t readLen = fread(SBD+sbdLen*sectorSize, 1, sectorSize, newfile);
                        (void)readLen;
			sbdLen++;
			if (sbdLen >= sbdMaxLen) {
			...
			sbdCurrent = NumUtils::getlong(BBD, sbdCurrent*4);
			if(sbdCurrent < 0 ||
	...
	propLen = 0;
	propMaxLen = 5;
	propCurrent = propStart = NumUtils::getlong(oleBuf,0x30);
	if (propStart >= 0) {
		...
		while(1) {
			...
			size_t readLen = fread(properties+propLen*sectorSize,
			...
			propCurrent = NumUtils::getlong(BBD, propCurrent*4);
			if(propCurrent < 0 ||
...
* @return 
*/
int Ole::rightOleType(unsigned char *oleBuf) {
...
* @return 
*/
oleType Ole::getOleType(unsigned char *oleBuf) {
...
FILE * Ole::ole_readdir(FILE *f) {
	...
	e->file=f;
	e->startBlock=NumUtils::getlong(oleBuf,0x74);
	e->blocks=NULL;

	nLen=NumUtils::getshort(oleBuf,0x40);
	for (i=0 ; i < nLen /2; i++)
		e->name[i]=(char)oleBuf[i*2];
	e->name[i]='\0';
	propCurNumber++;
	e->length=NumUtils::getulong(oleBuf,0x78);
	...
	if (e->startBlock >= 0 &&
		...
		fileLength/(e->isBigBlock ? sectorSize : shortSectorSize))) {
			if((e->blocks=(long*)malloc(chainMaxLen*sizeof(long int))) == NULL ) {
				return NULL;
			}
			while(1) {
				...
				if (e->numOfBlocks >= chainMaxLen) {
					long int *newChain;
					chainMaxLen+=25;
					if ((newChain=(long*)realloc(e->blocks,
				...
				if ( e->isBigBlock ) {
					chainCurrent = NumUtils::getlong(BBD, chainCurrent*4);
				} else if ( SBD != NULL ) {
					chainCurrent = NumUtils::getlong(SBD, chainCurrent*4);
				} else {
					chainCurrent=-1;
				}
				if(chainCurrent <= 0 ||
					chainCurrent >= ( e->isBigBlock ?
                                                ...
                                                : (long int)((sbdNumber*shortSectorSize)/4) ) ||
					(e->numOfBlocks >
                                                (long int)e->length/(e->isBigBlock ? (long int)sectorSize : (long int)shortSectorSize))) {
        ...
        if((long int)e->length > (e->isBigBlock ? sectorSize : shortSectorSize)*e->numOfBlocks)
...
* @return 
*/
long int Ole::calcFileBlockOffset(oleEntry *e, long int blk) {
	long int res;
	if ( e->isBigBlock ) {
...
*/
size_t Ole::ole_read(void *ptr, size_t size, size_t nmemb, FILE *stream) {
	if(m_bStdFunc)
	...
	if( e->ole_offset+llen > (long int)e->length )
...
int Ole::ole_eof(FILE *stream) {
	...
	return (e->ole_offset >=  (long int)e->length);
}
...
* @return 
*/
int Ole::ole_seek(FILE *stream, long offset, int whence) {
	if(m_bStdFunc)
	...
	if(new_ole_offset<0)
		new_ole_offset=0;
	if(new_ole_offset >= (long int)e->length)
...
void Ole::set_ole_func(void) {
	m_bStdFunc = false;	
}

void Ole::set_std_func(void) {
	m_bStdFunc = true;	
}
