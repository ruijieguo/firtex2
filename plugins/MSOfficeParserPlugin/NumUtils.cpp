 ...
 * way
 *********************************************************************/ 
unsigned int NumUtils::getshort(unsigned char *buffer,int offset) {
 ...
 * way
 *********************************************************************/ 
long int NumUtils::getlong(unsigned char *buffer,int offset) {
	return (long)buffer[offset]|((long)buffer[offset+1]<<8L)
		|((long)buffer[offset+2]<<16L)|((long)buffer[offset+3]<<24L);
}  

unsigned long int NumUtils::getulong(unsigned char *buffer,int offset) {
