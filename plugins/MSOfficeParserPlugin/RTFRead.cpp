...
RTFRead::RTFRead(Analyze* pAnalyze)
...
void RTFRead::add_to_buffer(int *bufptr,unsigned short int c) {
	buffer[++(*bufptr)]=c;
	if (*bufptr > PARAGRAPH_BUFFER2-2 
            && *bufptr < PARAGRAPH_BUFFER2 - 1) {
            buffer[++(*bufptr)]=0;
...
void RTFRead::end_paragraph(int *bufptr) {
 ...
 */
signed long RTFRead::getNumber(FILE *f) {
	int c,count=0;
 ...
 */
int RTFRead::getRtfCommand(FILE *f, RTFcommand *command ) {
	int c=fgetc(f);
	if (isalpha(c)) {
		int name_count=1;
 ...
 */
unsigned short int RTFRead::rtf_to_unicode(int code) {
	int cc=code;
	if (code < 0 || (cc=m_pAnalyze->getCharsets()->to_unicode(current_charset, code)) < 0 ) return 0xFEFF;
	return cc;
}
 ...
 */
RTFTypes RTFRead::getCommandType(char *name) {
	int i, olen=sizeof(rtf_types)/sizeof(RTFTypeMap);
	for (i = 0; i < olen ; i++) {
            if ( strcmp(name,rtf_types[i].name.c_str()) == 0 ) {
 ...
 * @return converted number
 */
signed int RTFRead::getCharCode(FILE *f) {
	int c,count=0,i;
	char buf[RTFARGSMAXLEN];
	for(i=0;i<2; i++) {
...
void RTFRead::rtfSetCharset(short int **charset_ptr,unsigned int codepage)
{
	/* Do not override charset if it is specified in the command line */
	const char *charset_name;
//	char *save_buf = input_buffer;
	if (GlobalConf.forced_charset) return;
	charset_name = Charsets::charset_from_codepage(codepage);
	char* source_csname = GlobalConf.source_csname;
	FileUtil::check_charset(&source_csname,charset_name,GlobalConf.charset_path);
//	input_buffer=NULL;
	*charset_ptr = m_pAnalyze->getCharsets()->read_charset();	
